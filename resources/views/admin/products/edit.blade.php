...
<div class="min-h-screen py-8" style="background-color: #fefae0;">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8">
            <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea name="description_en" id="description_en" rows="5"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#7b8f5a] focus:border-transparent"
                                placeholder="Enter product description in English..." required>{{ old('description_en', $product->description_en) }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea name="description_ar" id="description_ar" rows="5"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#7b8f5a] focus:border-transparent"
                                placeholder="أدخل وصف المنتج باللغة العربية..." required dir="rtl">{{ old('description_ar', $product->description_ar) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="status" 
                                    ...
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#7b8f5a] focus:border-transparent"
                                    required>
                                <option value="new_arrival" {{ old('status', $product->status) == 'new_arrival' ? 'selected' : '' }}>New Arrival</option>
                                <option value="best_selling" {{ old('status', $product->status) == 'best_selling' ? 'selected' : '' }}>Best Selling</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <!-- Product Colors -->
                    <div x-data="{ colors: {{ json_encode(old('colors', $product->colors ?? [])) }} }">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div>
                                ...
                                <input type="number" name="width" id="width" value="{{ old('width', $product->width) }}" step="0.01"
                                    min="0"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#7b8f5a] focus:border-transparent"
                                    placeholder="0.00">
                                @error('width')
                            ...
                            <div>
                                ...
                                <input type="number" name="height" id="height" value="{{ old('height', $product->height) }}" step="0.01"
                                    min="0"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#7b8f5a] focus:border-transparent"
                                    placeholder="0.00">
                                @error('height')
                            ...
                            <div>
                                ...
                                <input type="number" name="depth" id="depth" value="{{ old('depth', $product->depth) }}" step="0.01"
                                    min="0"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#7b8f5a] focus:border-transparent"
                                    placeholder="0.00">
                                @error('depth')
                            ...
                            <div>
                                ...
                                <select name="dimension_unit" id="dimension_unit"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#7b8f5a] focus:border-transparent">
                                    <option value="cm" {{ old('dimension_unit', $product->dimension_unit ?? 'cm') == 'cm' ? 'selected' : '' }}>cm</option>
                                    <option value="m" {{ old('dimension_unit', $product->dimension_unit) == 'm' ? 'selected' : '' }}>m</option>
                                    <option value="inch" {{ old('dimension_unit', $product->dimension_unit) == 'inch' ? 'selected' : '' }}>inch</option>
                                    <option value="ft" {{ old('dimension_unit', $product->dimension_unit) == 'ft' ? 'selected' : '' }}>ft</option>
                                </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-[#7b8f5a] transition duration-300 cursor-pointer"
                                ...
                                <div id="uploadPlaceholder" class="{{ $product->image ? 'hidden' : '' }}">
                                    ...
                                    <p class="text-gray-700 font-semibold mb-2">Click or drop to replace main image</p>
                                </div>

                                <input type="file" name="image" id="image" accept="image/*" class="hidden" onchange="previewMainImage(event)">
                            </div>
                            @error('image')
                        ...
                        <div class="bg-gray-50 rounded-xl p-6 border border-gray-100">
                            <h3 class="font-bold text-gray-800 mb-4 flex items-center">
                                <i class="fas fa-info-circle mr-2 text-[#7b8f5a]"></i>
                                Image Guidelines
                            </h3>
                            <ul class="text-sm text-gray-600 space-y-2">
                                ...
                                <li>• Main image is used in listings</li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div>
                                    ...
                                    <div class="relative group aspect-square">
                                        ...
                                        <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition rounded-lg flex items-center justify-center">
                                            <button type="button" 
                                                    ...
                                                    class="bg-red-500 text-white w-8 h-8 rounded-full hover:bg-red-600 transition flex items-center justify-center">
                        ...
                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-[#7b8f5a] transition duration-300 cursor-pointer"
                            ...
                            <div id="galleryUploadPlaceholder">
                                ...
                                <p class="text-gray-700 font-semibold mb-2">Add more gallery images</p>
                                <p class="text-sm text-gray-500">Click or drop multiple images</p>
                            </div>
        ...
        <form id="deleteGalleryForm" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
